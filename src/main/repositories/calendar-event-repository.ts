...
export class CalendarEventRepository {
  ...
  list(range: CalendarRange): CalendarEventRow[] {
    return this.db
      .prepare(
        `${SELECT}
      WHERE e.start_at < ?
        AND (e.end_at > ? OR (e.start_at = e.end_at AND e.start_at >= ?))
      ORDER BY e.start_at, e.id
    `,
      )
      .all(range.endAt, range.startAt, range.startAt) as CalendarEventRow[]
  }
  get(id: number): CalendarEventRow | undefined {
    return this.db.prepare(`${SELECT} WHERE e.id = ?`).get(id) as CalendarEventRow | undefined
  }
  ...
  listDue(currentAt: number): Array<{
    event_id: number
    title: string
    start_at: number
    end_at: number
  ...
  markReminderSent(eventId: number, reminderAt: number, sentAt: number): void {
    this.db.transaction(() => {
      this.db
        .prepare(
          'INSERT OR IGNORE INTO calendar_event_reminders (calendar_event_id, reminder_at, sent_at) VALUES (?, ?, ?)',
        )
        .run(eventId, reminderAt, sentAt)
    })()
  }
  map(row: CalendarEventRow) {
