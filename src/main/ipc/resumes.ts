...
export function registerResumeIpc(services: Services): void {
  ...
  registerChannel('resumes:get', (id) => services.resumes.get(numberValue(id, '简历版本 ID')))
  ...
  registerChannel('resumes:open', async (id) => {
    const result = await shell.openPath(services.resumes.getPath(numberValue(id, '简历版本 ID')))
    if (result) throw new AppServiceError('FILE_OPEN_FAILED', result)
  })
  registerChannel('resumes:update', (id, input) =>
    services.resumes.update(numberValue(id, '简历版本 ID'), parseResumeUpdate(input)),
  )
  registerChannel('resumes:reorder', (order) =>
    services.resumes.reorder(ids(order, '简历版本顺序')),
  )
  registerChannel('resumes:delete', (id) => services.resumes.delete(numberValue(id, '简历版本 ID')))
}
