...
export function registerIndustryIpc(services: Services): void {
  ...
  registerChannel('industries:get', (id) => services.industries.get(numberValue(id, '行业分类 ID')))
  registerChannel('industries:create', (input) =>
    services.industries.create(parseIndustry(input, false)),
  )
  registerChannel('industries:update', (id, input) =>
    services.industries.update(numberValue(id, '行业分类 ID'), parseIndustry(input, true)),
  )
