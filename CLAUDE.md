...
- `package.json` 是唯一的产品版本来源；Rust 构建脚本直接读取它并写入安装器及可执行文件资源。Cargo 包版本是内部元数据，不代表产品版本。
...
- macOS 或 Linux 发行版。
- 账号、登录、云同步、多设备同步或遥测。
- 数据导入导出或通用自动备份。首次正式版不提供测试版数据迁移；未来正式版改变持久化结构时必须提供从已发布版本升级的迁移。
...
- 当前 v0.9.0 是正式版前过渡版本，v1.0.0 计划作为第一个正式版；数据库和配置格式分别以 `PRAGMA user_version = 1`、`configVersion = 1` 为基线。软件版本升级不要求数据版本升级。
- 仅 `user_version = 0` 的全新数据库执行完整初始化和 seed。已初始化的版本 1 数据库不得再执行建表 SQL。测试版数据不提供迁移或兼容代码。
- v1.0.0 发布后，数据库的表、列、索引或约束，以及配置的持久化结构变化，分别递增对应版本并提供从已发布正式版升级的迁移；格式不变则版本不变。迁移须保持用户数据并在失败时回滚。正式版发布前应固定 v1 数据库和配置样本；发布后不得重写该样本。
...
- 缺失配置时创建默认配置。损坏、已知字段无效或版本不受支持时保留原文件，不写默认值；桌面、MCP 和根启动器以退出码 78 静默退出。更新过程中先执行现有回滚，再决定是否退出。
...
- 相邻且独立的本地只读调用由 `ToolNode` 并行执行；`read_web_page` 可能触发同站 POST，须逐项执行。
...
- 生成快照前创建 `.runtime/state/update-freeze`，阻止所有新业务事务和配置修改；已运行的 MCP 进程检测后关闭，更新流程等待其租约全部释放，并等待既有写事务结束，随后才复制配置和数据库。准备或调用更新器失败时清除冻结标记。
- 待更新状态写入 `.runtime/state/pending-update.json`；回滚点提交前不得启动新版本。
- 根启动器在待更新状态或冻结标记存在时不得启动新的 MCP 服务；目标版本已安装时先完成健康检查或回滚。成功启动或回滚后清除冻结标记。仍为来源版本且待更新时不得将来源版本误判为更新成功。
...
本地使用 `pnpm release:win`。发布流程必须：

1. 从固定 Feed 选择低于目标版本且不早于 v1.0.0 的最新正式版 Full 包；首个正式版不使用测试版 Full 包。
2. 校验历史包大小和 SHA-256。
