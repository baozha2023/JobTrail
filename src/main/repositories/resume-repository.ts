...
export class ResumeRepository {
  ...
  list(): ResumeRow[] {
    return this.db
      .prepare('SELECT * FROM resume_versions ORDER BY sort_order, id')
      .all() as ResumeRow[]
  }
  get(id: number): ResumeRow | undefined {
    return this.db.prepare('SELECT * FROM resume_versions WHERE id = ?').get(id) as
      | ResumeRow
      | undefined
  }
  maxSortOrder(): number {
    return (
      this.db
        .prepare('SELECT COALESCE(MAX(sort_order), -1) AS max_order FROM resume_versions')
  ...
  update(id: number, input: UpdateResumeVersionInput, current: ResumeRow, timestamp: number): void {
    this.db.transaction(() => {
      this.db
        .prepare('UPDATE resume_versions SET name = ?, note = ?, updated_at = ? WHERE id = ?')
        .run(
  ...
  reorder(order: number[], timestamp: number): void {
    const update = this.db.prepare(
      'UPDATE resume_versions SET sort_order = ?, updated_at = ? WHERE id = ?',
    )
    this.db.transaction(() => {
  ...
  delete(id: number): number {
    return this.db.transaction(
      () => this.db.prepare('DELETE FROM resume_versions WHERE id = ?').run(id).changes,
    )()
  }
  countUsage(id: number): number {
    return (
      this.db
        .prepare('SELECT COUNT(*) AS count FROM opportunities WHERE resume_version_id = ?')
